...
Design rules behind ShinobiData — protocol surface, data flow, and load-bearing decisions that keep responses fast and narratable.
...
Hosted service. Users never run anything locally. The MCP endpoint at `https://mcp.shinobidata.com/api/mcp/mcp` is publicly reachable; OAuth gates per-tool access.
...
- **Refresh token rotates** on every use; old token revoked atomically (RFC 6749 §10.4).
- **Authorization codes are single-use**, 10-minute TTL.
- **Tokens stored as SHA-256 hashes**, never plaintext.
- **Three scopes**: `portfolio:read`, `portfolio:write`, `market:read` — least-privilege.

---

## Why our analytics tools don't paginate

> **Heavy lifting belongs in SQL, not in the model's context.**

If a tool returned 10 rows per call and a user had a 200-holding portfolio, the AI would call the tool 20 times and stitch results. Mechanically it works; in practice it's the wrong design.
...
**What we do instead — server aggregates, returns summaries:**
...
~500 bytes regardless of portfolio size. One tool call. Constant cost.

| Tool type | Row cap | Why |
|---|---|---|
| Portfolio analytics | None — server aggregates | One summary covers any portfolio size |
| Market / sector tools | None — server aggregates | Same |
...
- **Every response includes** envelope `dataAsOf` (wallclock) + per-tool `snapshotAsOf` (underlying data freshness). Distinct on purpose.
...
- ChatGPT deep-research connector spec (canonical `search` + `fetch` tool shapes)
